#ifndef CT_ICP_ODOMETRY__ROSCORE__POINT_TYPES_HPP
#define CT_ICP_ODOMETRY__ROSCORE__POINT_TYPES_HPP
...
struct XYZTPoint
{
  ...
  inline XYZTPoint(const XYZTPoint & pt)
  : x(pt.x), y(pt.y), z(pt.z), timestamp(pt.timestamp)
  {
    data[3] = 1.0f;
  }
...
struct LidarPoint
{
  ...
  inline LidarPoint(const LidarPoint & p)
  : x(p.x), y(p.y), z(p.z), time(p.time), intensity(p.intensity),
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
  ...
  (float, x, x)(float, y, y)(float, z, z)(double, time, time)(
    float, intensity,
    intensity)(std::uint16_t, laser_id, laser_id)(std::uint8_t, device_id, device_id)(
    std::uint8_t,
    label, label))

#endif // CT_ICP_ODOMETRY__ROSCORE__POINT_TYPES_HPP
