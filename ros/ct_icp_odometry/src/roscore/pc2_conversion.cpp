...
slam::ItemSchema::Builder SchemaBuilderFromCloud2(const sensor_msgs::msg::PointCloud2 & cloud)
{
  ...
  for (auto & field: fields) {
    ...
    if (field.offset != expected_offset) {
      builder.AddScalarProperty<char>(
        "properties", "padding_" + std::to_string(padding_idx++),
  ...
  std::shared_ptr<PointCloud2PtrWrapper> pointer)
{
  ...
  auto collection2 = slam::BufferCollection::Factory(
    std::make_unique<slam::BufferWrapper>(
