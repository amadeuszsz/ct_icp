...
slam::Pose ROSOdometryToPose(const nav_msgs::msg::Odometry & odometry, slam::frame_id_t frame_id)
{
  ...
  pose.dest_timestamp = odometry.header.stamp.sec;
...
PoseToROSOdometry(
  const Pose & pose, const std::string & src_frame_id,
  const std::string & dest_frame_id)
{
  ...
  odometry.header.stamp.nanosec = uint32_t(
    (pose.dest_timestamp - double(odometry.header.stamp.sec)) * 1.e9);
...
sensor_msgs::msg::Imu SlamToROSImu(const ImuData & imu)
{
  ...
  ros_imu.header.stamp.nanosec = uint32_t(
    (imu.time_seconds - double(ros_imu.header.stamp.sec)) * 1.e9);
